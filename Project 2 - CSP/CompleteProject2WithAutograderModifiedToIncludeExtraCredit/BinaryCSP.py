...
"""
    Implementation of BinaryConstraint
    Satisfied if both values assigned are not on diagonals of one another in a grid
"""
class NotSameDiagonalsConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
        if abs(int(self.var1[1:]) - int(self.var2[1:])) == abs(int(value1) - int(value2)):
            return False
        return True

    def __repr__(self):
        return 'NotSameDiagonalsConstraint (%s, %s)' % (str(self.var1), str(self.var2))

"""
    Implementation of BinaryConstraint
    Satisfied if both values assigned are different and if both variable numbers are different
"""
class NotSameRowColConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
        if value1 == value2:
            return False
        elif int(self.var1[1:]) == int(self.var2[1:]):
            return False
        return True

    def __repr__(self):
        return 'NotSameRowColConstraint (%s, %s)' % (str(self.var1), str(self.var2))
...
def consistent(assignment, csp, var, value):
    """Question 1"""
    output = True
    listOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(var)]
    for BP in listOfAffectedBC:
        complementVar = BP.otherVariable(var)
        if assignment.isAssigned(complementVar):
            output = output and BP.isSatisfied(value, assignment.assignedValues[complementVar])
    return output
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
    ...
    if assignment.isComplete():
        return assignment;
    currentVar = selectVariableMethod(assignment, csp)
    for value in orderValuesMethod(assignment, csp, currentVar):
        if consistent(assignment, csp, currentVar, value):
            assignment.assignedValues[currentVar] = value
            result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
            if result is not None:
                return result
        assignment.assignedValues[currentVar] = None
    return None
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    currentSmallestDomainValue = len(domains) + 1
    for variable in variables:
        if not assignment.isAssigned(variable):
            if currentSmallestDomainValue > len(domains[variable]):
                minimumRemainingValues = []
                currentSmallestDomainValue = len(domains[variable])
                minimumRemainingValues.append(variable)
            elif currentSmallestDomainValue == len(domains[variable]):
                minimumRemainingValues.append(variable)
    if len(minimumRemainingValues) == 1:
        return minimumRemainingValues[0]
    currentLargestDegree = 0
    for variable in minimumRemainingValues:
        if len(csp.binaryConstraints) == 0:
            return minimumRemainingValues[0]
        currentDegree = len([BC for BC in csp.binaryConstraints if BC.affects(variable)])
        if currentDegree > currentLargestDegree:
            nextVar = variable
            currentLargestDegree = currentDegree
    return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    output = []
    for value in values:
        numOfConstrainedValues = countConstrainedValues(assignment, csp, var, value)
        output.append((numOfConstrainedValues, value))
    output.sort()
    values = [x[1] for x in output]
    return values

def countConstrainedValues(assignment, csp, var, value): #Helper method for leastConstrainingValuesHeuristic
    affectedVariables = []
    listOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(var)]
    for BP in listOfAffectedBC:
        complementVar = BP.otherVariable(var)
        for otherValue in assignment.varDomains[complementVar]:
            if not BP.isSatisfied(value, otherValue):
                poorAssignment = (complementVar, otherValue)
                if poorAssignment not in affectedVariables:
                    affectedVariables.append(poorAssignment)
    return len(affectedVariables)
...
def forwardChecking(assignment, csp, var, value):
    ...
    domains = assignment.varDomains
    listOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(var)]
    for BC in listOfAffectedBC:
        complementVariable = BC.otherVariable(var)
        complementUnsatisfiedValues = [val for val in domains[complementVariable] if not BC.isSatisfied(value, val)]
        for complementUnsatisfiedValue in complementUnsatisfiedValues:
            domains[complementVariable].remove(complementUnsatisfiedValue)
            inferences.add((complementVariable, complementUnsatisfiedValue))
            if len(domains[complementVariable]) <= 0:
                for complementVariable, complementUnsatisfiedValue in inferences:
                    domains[complementVariable].add(complementUnsatisfiedValue)
                return None
    return inferences
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    for value in orderValuesMethod(assignment, csp, currentVar):
        if consistent(assignment, csp, currentVar, value):
            assignment.assignedValues[currentVar] = value
            inferences = inferenceMethod(assignment, csp, currentVar, value)
            if inferences is not None:
                result = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
                if result is not None:
                    return result
                for complementVariable, complementValue in inferences:
                    assignment.varDomains[complementVariable].add(complementValue)
            assignment.assignedValues[currentVar] = None
    return None
        ...
        for value2 in domains[var2]:
            ...
            for value1 in domains[var1]:
                if (constraint.isSatisfied(value1, value2)):
                    constraintSatisfied = constraintSatisfied or True
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    listOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(var)]
    for BC in listOfAffectedBC:
        complementVar = BC.otherVariable(var)
        if (assignment.assignedValues[complementVar] is None):
            queue.put((var, complementVar, BC))
    while(not queue.empty()):
        arc = queue.get()
        variableSpecificInferences = revise(assignment, csp, arc[0], arc[1], arc[2])
        if (variableSpecificInferences is None):
            for variable, value in inferences:
                assignment.varDomains[variable].add(value)
            return None
        if (len(variableSpecificInferences) != 0):
            newListOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(arc[1])]
            for newBC in newListOfAffectedBC:
                newComplementVar = newBC.otherVariable(arc[1])
                if (assignment.assignedValues[newComplementVar] is None):
                    queue.put((arc[1], newComplementVar, newBC))
            inferences = inferences.union(variableSpecificInferences)
    return inferences
...
def AC3(assignment, csp):
    ...
    """Hint: implement revise first and use it as a helper function"""
    queue = Queue.Queue()
    variables = csp.varDomains.keys()
    for variable in variables:
        listOfAffectedBC = [BC for BC in csp.binaryConstraints if BC.affects(variable)]
        for BC in listOfAffectedBC:
            complementVar = BC.otherVariable(variable)
            queue.put((variable, complementVar, BC))
    while (not queue.empty()):
        arc = queue.get()
        variableSpecificInferences = revise(assignment, csp, arc[0], arc[1], arc[2])
        if (variableSpecificInferences is None):
