...
def getAverageClassificaionRate(dataset,runs=20,testSize=25,setFunc = setEntropy, infoFunc = infoGain):
    """
...
def testExtraCredit(setFunc = setEntropy, infoFunc = infoGain):
    ...
    print 'Testing Iris-Rounded dataset. Number of examples %d.'%len(examples)
    tree = makeTree(examples, attrValues, labelName, setFunc, infoFunc)
    f = open('iris_rounded.out','w')
    f.write(str(tree))
    f.close()
    print 'Tree size: %d.\n'%tree.count()
    print 'Entire tree written out to iris_rounded.out in local directory\n'
...
testExtraCredit(setFunc = setEntropy, infoFunc = infoGain)