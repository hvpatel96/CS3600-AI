...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    pertinentChildren = self.root.children[classificationData[self.root.attr]]
    subTree = Tree(pertinentChildren)
...
def getPertinentExamples(examples,attrName,attrValue):
    ...
    """
    newExamples = [ex for ex in examples if ex[attrName] == attrValue]
    return newExamples
...
def setEntropy(classCounts):
    ...
    for count in classCounts:
        probCount = float(count) / sum(classCounts)
        entropy += -(probCount * log(probCount, 2))
    return entropy
...
def remainder(examples,attrName,attrValues,className):
    ...
    totalOfClassCounts = sum(classCounts.values())
    for value in attrValues:
        pertinentExamplesForValue = getPertinentExamples(examples,attrName,value)
        valueClassCounts = getClassCounts(pertinentExamplesForValue, className)
        totalOfValueClassCounts = sum(valueClassCounts.values())
        remainder += (float(totalOfValueClassCounts) / totalOfClassCounts * setEntropy(valueClassCounts.values()))
    return remainder
...
def giniIndex(classCounts):
    ...
    """
    gIndex = 1;
    for classC in classCounts:
        gIndex -= ((float(classC) / sum(classCounts)) ** 2)
    return gIndex
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for value in attrValues:
        pertinentExamplesForValue = getPertinentExamples(examples,attrName,value)
        valueClassCounts = getClassCounts(pertinentExamplesForValue, className)
        gGain += float(len(pertinentExamplesForValue)) / len(examples) * giniIndex(valueClassCounts.values())
    if gGain == 0:
        return sys.maxint
    inverseGGain = 1.0 / gGain
    return inverseGGain
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    if len(examples) == 0:
        ...
        return root
    classCounts = getClassCounts(examples, className)
    if classCounts[examples[0][className]] == len(examples):
        root = LeafNode(examples[0][className])
        return root
    if len(remainingAttributes) == 0:
        root = LeafNode(getMostCommonClass(examples, className))
        return root
    bestRemainingAttribute = remainingAttributes[0]
    for attribute in remainingAttributes:
        if gainFunc(examples, attribute, attributeValues[attribute], className) > gainFunc(examples, bestRemainingAttribute, attributeValues[bestRemainingAttribute], className):
            bestRemainingAttribute = attribute
    secondList = list(remainingAttributes)
    secondList.remove(bestRemainingAttribute)
    root = Node(bestRemainingAttribute)
    for value in attributeValues[bestRemainingAttribute]:
        pertinentExamplesForValue = getPertinentExamples(examples,bestRemainingAttribute,value)
        root.children[value] = makeSubtrees(secondList, pertinentExamplesForValue, attributeValues, className, getMostCommonClass(examples, className), setScoreFunc, gainFunc)
    return root
