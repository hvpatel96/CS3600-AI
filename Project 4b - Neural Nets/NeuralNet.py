...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            nextNodesActs = []
            for node in layer:
                nextNodesActs.append(node.sigmoidActivation(inActs))
            outputActs.append(nextNodesActs)
            inActs = nextNodesActs
        return outputActs
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        errorSum = errorSum + nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                    delta = gPrime * errorSum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for exampleIn, exampleOut in examplesTest:
        ...
        for vIndex in xrange(len(outputList)):
            if round(outputList[vIndex]) != exampleOut[vIndex]:
                allOutputsMatch = False
                break
        if allOutputsMatch:
            testCorrect += 1
    testError = totalValue - testCorrect
    testAccuracy = testCorrect * 1.0 / totalValue
